...
class TestMVPECommerceFlow:
    ...
    async def test_complete_mvp_flow(
        ...
        profile_response = await client.get(
            f"/api/customers/profile/{sample_customer_data['email']}"
        )
        ...
        our_product = next((p for p in browse_data["products"] if p["id"] == product_id), None)
        ...
        result = await sqlite_session.execute(
            text(
                "SELECT order_id, product_id, quantity FROM order_items WHERE order_id = :order_id"
            ),
            {"order_id": order_id},
        )
