...
class DomainEventDispatcher:
    ...
    def register_handler(self, event_type: type[DomainEvent], handler: Any) -> None:
        ...
        if not issubclass(event_type, DomainEvent):
            logger.error(
                f"Attempted to register handler for non-DomainEvent type: {event_type.__name__}"
            )
            return
        ...
        logger.info(
            f"[DomainEventDispatcher] Registered internal handler '{handler_name}' for event '{event_type.__name__}'"
        )
    ...
    async def dispatch(self, event: DomainEvent) -> None:
        ...
        logger.info(
            f"[DomainEventDispatcher] Dispatching internal event: {event_type.__name__} (ID: {event.id if hasattr(event, 'id') else 'N/A'})"
        )
        ...
        if event_type in self._handlers:
            for handler in self._handlers[event_type]:
                handler_name = (
                    handler.__class__.__name__ if hasattr(handler, "__class__") else str(handler)
                )
                logger.debug(
                    f"[DomainEventDispatcher] Executing internal handler '{handler_name}' for '{event_type.__name__}'"
                )
                try:
                    await handler.handle(event)  # Assuming handlers have an async 'handle' method
                except Exception as e:
                    logger.error(
                        f"[DomainEventDispatcher] Error in internal handler '{handler_name}' for event '{event_type.__name__}': {e}",
                        exc_info=True,
                    )
        ...
        if DomainEvent in self._handlers and event_type is not DomainEvent:
            for handler in self._handlers[DomainEvent]:
                ...
                logger.debug(
                    f"[DomainEventDispatcher] Executing base DomainEvent handler '{handler_name}' for specific event '{event_type.__name__}'"
                )
                try:
                    await handler.handle(event)
                except Exception as e:
                    logger.error(
                        f"[DomainEventDispatcher] Error in base DomainEvent handler '{handler_name}' for event '{event_type.__name__}': {e}",
                        exc_info=True,
                    )
        ...
        # If the event itself is of type DomainEvent (e.g. a generic event was dispatched)
            ...
            for handler in self._handlers[DomainEvent]:
                ...
                logger.debug(
                    f"[DomainEventDispatcher] Executing handler '{handler_name}' for generic DomainEvent"
                )
                try:
                    await handler.handle(event)
                except Exception as e:
                    logger.error(
                        f"[DomainEventDispatcher] Error in handler '{handler_name}' for generic DomainEvent: {e}",
                        exc_info=True,
                    )

        if event_type not in self._handlers and (
            event_type is not DomainEvent or DomainEvent not in self._handlers
        ):
            logger.debug(
                f"[DomainEventDispatcher] No specific or base handlers registered for internal event '{event_type.__name__}'"
            )
