...
class UpdateOrderOnOrderProcessing:
    ...
    async def handle(self, event: OrderProcessingEvent) -> None:
        logger.info(
            f"[UpdateOrderOnOrderProcessingHandler] Updating read model for order {event.order_number} to status {event.status}"
        )
        ...
        if order_dto:
            ...
            logger.info(
                f"[UpdateOrderOnOrderProcessingHandler] Successfully updated read model for order {event.order_number}."
            )
        else:
            logger.warning(
                f"[UpdateOrderOnOrderProcessingHandler] OrderDetailsDTO not found for order ID {event.aggregate_id}. Cannot update read model."
            )
