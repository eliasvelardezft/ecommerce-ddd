...
class UpdateOrderOnOrderCancelled:
    ...
    async def handle(self, event: OrderCancelledEvent) -> None:
        logger.info(
            f"[UpdateOrderOnOrderCancelledHandler] Updating read model for order {event.order_number} to status {event.status}"
        )
        if event.cancellation_reason:
            logger.info(
                f"[UpdateOrderOnOrderCancelledHandler] Cancellation reason: {event.cancellation_reason}"
            )
        ...
        if order_dto:
            ...
            logger.info(
                f"[UpdateOrderOnOrderCancelledHandler] Successfully updated read model for order {event.order_number}."
            )
        else:
            logger.warning(
                f"[UpdateOrderOnOrderCancelledHandler] OrderDetailsDTO not found for order ID {event.aggregate_id}. Cannot update read model."
            )
