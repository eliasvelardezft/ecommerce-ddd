...
class ShippingDetails(BaseModel):
    ...
    address_line2: Annotated[str, Field(max_length=255)] | None = (
        None  # Optional can wrap Annotated
    )
    ...
    def validate_phone_number(cls, value: str) -> str:
        ...
        if not cleaned.isdigit():
            raise ValueError(
                "Phone number must contain only digits and optional '+', '-', ' ', '(', ')' characters"
            )
        return value
    ...
    def validate_postal_code(cls, value: str) -> str:
        cleaned = value.replace(" ", "").replace(
            "-", ""
        )  # Allow hyphens in postal codes before checking isalnum
        if not cleaned.isalnum():
            raise ValueError(
                "Postal code must contain only alphanumeric characters, spaces, and hyphens (hyphens are removed for validation check but allowed in original)."
            )
        return value
