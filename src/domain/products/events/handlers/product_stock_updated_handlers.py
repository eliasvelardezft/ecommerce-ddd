...
class UpdateProductOnProductStockUpdated:
    ...
    async def handle(self, event: ProductStockUpdatedEvent) -> None:
        logger.info(
            f"Updating product stock on event: {event.aggregate_id}, new stock: {event.stock_quantity}"
        )
        ...
        if product_dto:
            ...
            logger.warning(
                f"ProductDetailsDTO not found for product ID {event.aggregate_id} during stock update."
            )
