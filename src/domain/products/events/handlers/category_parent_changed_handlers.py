...
class UpdateCategoryOnCategoryParentChanged:
    ...
    async def _update_child_category_parent_id(
        self, child_category_id: UUID, event: CategoryParentChangedEvent
    ) -> None:
        ...
        if not child_category:
            logger.warning(
                f"CategoryDetailsDTO not found for child category ID {child_category_id}. Aborting."
            )
            return
        ...
        logger.info(
            f"Successfully updated parent_category_id for child category {child_category_id} to {event.new_parent_category_id}"
        )

    async def _update_prev_parent_children_ids(
        self, child_category_id: UUID, event: CategoryParentChangedEvent
    ) -> None:
        prev_parent = await self._read_repository.get_category(
            category_id=event.prev_parent_category_id
        )
        if prev_parent:
            if child_category_id in prev_parent.children_ids:
                ...
                logger.info(
                    f"Removed child {child_category_id} from previous parent {event.prev_parent_category_id}'s children_ids."
                )
            else:
                logger.warning(
                    f"Child {child_category_id} not found in previous parent {event.prev_parent_category_id}'s children_ids. No removal needed."
                )
        else:
            logger.warning(
                f"Previous parent DTO {event.prev_parent_category_id} not found. Cannot update its children_ids."
            )

    async def _update_new_parent_children_ids(
        self, child_category_id: UUID, event: CategoryParentChangedEvent
    ) -> None:
        new_parent = await self._read_repository.get_category(
            category_id=event.new_parent_category_id
        )
        if new_parent:
            if child_category_id not in new_parent.children_ids:
                ...
                logger.info(
                    f"Added child {child_category_id} to new parent {event.new_parent_category_id}'s children_ids."
                )
            else:
                logger.info(
                    f"Child {child_category_id} already present in new parent {event.new_parent_category_id}'s children_ids. No addition needed."
                )
        else:
            logger.warning(
                f"New parent DTO {event.new_parent_category_id} not found. Cannot update its children_ids."
            )
    ...
    async def handle(self, event: CategoryParentChangedEvent) -> None:
        ...
        logger.info(
            f"Finished handling CategoryParentChangedEvent for category {child_category_id}"
        )
