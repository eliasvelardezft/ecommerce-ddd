...
class UpdateCategoryOnCategoryDetailsUpdated:
    ...
    async def handle(self, event: CategoryDetailsUpdatedEvent) -> None:
        ...
        category_dto = await self._read_repository.get_category(
            category_id=UUID(event.aggregate_id)
        )
        ...
        if category_dto:
            ...
            logger.warning(
                f"CategoryDetailsDTO not found for category ID {event.aggregate_id} during details update."
            )
