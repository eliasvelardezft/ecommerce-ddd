...
class UpdateProductOnProductActivated:
    ...
    async def handle(self, event: ProductActivatedEvent) -> None:
        ...
        product_dto = await self._read_repository.get_product_details_by_id(
            product_id=UUID(event.aggregate_id)
        )
        ...
        if product_dto:
            ...
            logger.warning(
                f"ProductDetailsDTO not found for product ID {event.aggregate_id} during activation."
            )
