...
class UpdateProductOnProductPriceUpdated:
    ...
    async def handle(self, event: ProductPriceUpdatedEvent) -> None:
        logger.info(
            f"Updating product price on event: {event.aggregate_id}, new price: {event.price.amount} {event.price.currency}"
        )
        ...
        if product_dto:
            ...
            logger.warning(
                f"ProductDetailsDTO not found for product ID {event.aggregate_id} during price update."
            )
