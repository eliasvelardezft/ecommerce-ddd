...
class Category(AggregateRoot):
    ...
    def create(
        ...
        category = cls(
            _id=_id, name=name, description=description, parent_category_id=parent_category_id
        )
    ...
    def update_details(
        ...
        if name is not None and self.name != name:
            if not name.strip():
                raise ValueError(
                    "Category name cannot be empty when updating."
                )  # Replace with CategoryDomainException
            self.name = name
            changes["name"] = name
        ...
        if description is not None:
            ...
            description is None and self.description is not None
        ):  # If None is passed to clear existing description
        ...
        if changes:
            ...
            self.add_domain_event(
                CategoryDetailsUpdatedEvent(
                    aggregate_id=self.id, updated_details=changes, updated_at=self.updated_at
                )
            )

    def change_parent(self, new_parent_category_id: EntityId | None) -> None:
