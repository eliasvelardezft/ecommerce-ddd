...
class PlaceOrderHandler:
    ...
    async def handle(self, command: PlaceOrderCommand) -> Order:
        logger.info(
            f"[PlaceOrderHandler] Processing PlaceOrderCommand for customer {command.customer_id}"
        )
        ...
        logger.debug(
            f"[PlaceOrderHandler] Internal domain events generated by aggregate: {[ev.__class__.__name__ for ev in order.domain_events]}"
        )
        ...
        logger.info(
            f"[PlaceOrderHandler] Dispatched {len(order.domain_events)} domain events for order {order.id}."
        )
        ...
        for internal_event in order.domain_events:
            if internal_event.__class__ in self.integration_events:
                logger.info(
                    f"[PlaceOrderHandler] Publishing {internal_event.__class__.__name__} as a public contract for order {order.id}."
                )
