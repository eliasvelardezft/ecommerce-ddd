...
class UpdateCategoryDetailsHandler:
    ...
    async def handle(self, command: UpdateCategoryDetailsCommand) -> None:
        ...
        category.update_details(name=command.name, description=command.description)
        ...
        if category.domain_events:
            ...
            logger.info(
                f"Dispatched {len(category.domain_events)} domain events for category {category.id} after details update."
            )
            category.clear_domain_events()
        else:
            logger.info(
                f"No changes detected for category {command.category_id}. No update performed."
            )
