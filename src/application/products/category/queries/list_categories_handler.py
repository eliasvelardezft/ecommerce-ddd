...
class ListCategoriesHandler:
    ...
    async def handle(self, query: ListCategoriesQuery) -> list[CategoryDetailsDTO]:
        if query.parent_category_id:
            logger.info(
                f"Handling ListCategoriesQuery for children of parent ID: {query.parent_category_id}"
            )
            categories = await self._category_read_repository.list_children(
                query.parent_category_id
            )
            logger.info(
                f"Retrieved {len(categories)} children for parent ID: {query.parent_category_id}"
            )
        else:
