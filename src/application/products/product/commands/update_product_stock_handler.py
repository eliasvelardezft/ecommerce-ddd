...
class UpdateProductStockHandler:
    ...
    async def handle(self, command: UpdateProductStockCommand) -> None:
        logger.info(
            f"Handling UpdateProductStockCommand for Product ID: {command.product_id}, Change: {command.change_in_quantity}"
        )
        ...
        if command.change_in_quantity > 0:
            ...
            logger.info(
                f"Added {command.change_in_quantity} to stock for product {product.id}. New stock: {product.stock_quantity}"
            )
        elif command.change_in_quantity < 0:
            product.remove_stock(abs(command.change_in_quantity))
            logger.info(
                f"Removed {abs(command.change_in_quantity)} from stock for product {product.id}. New stock: {product.stock_quantity}"
            )
        else:
        ...
        logger.info(
            f"Dispatched {len(product.domain_events)} domain events for product {product.id} after stock update."
        )
