...
class UpdateProductPriceHandler:
    ...
    async def handle(self, command: UpdateProductPriceCommand) -> None:
        ...
        logger.info(
            f"Updated price for product {product.id} to {new_price.amount} {new_price.currency}."
        )
        ...
        logger.info(
            f"Dispatched {len(product.domain_events)} domain events for product {product.id} after price update."
        )
