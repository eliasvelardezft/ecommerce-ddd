...
class RegisterCustomerHandler:
    def __init__(
        self, write_repository: CustomerWriteRepository, event_dispatcher: DomainEventDispatcher
    ):
    ...
    async def handle(self, command: RegisterCustomerCommand) -> Customer:
        ...
        customer = Customer.create(name=command.name, email=command.email)
        ...
        logger.info(
            f"[Command] Dispatched {len(customer.domain_events)} domain events for customer {customer.id}"
        )
