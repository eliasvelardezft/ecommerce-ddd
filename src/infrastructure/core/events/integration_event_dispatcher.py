...
class IntegrationEventDispatcher:
    ...
    def __init__(self):
        ...
        logger.info(
            "[IntegrationEventDispatcher] Initialized for public integration event contracts."
        )
    ...
    async def dispatch(self, contract_event: PydanticBaseModel) -> None:
        ...
        logger.info(
            f"[IntegrationEventDispatcher] Dispatching public contract: {event_type_name_attr} (ID: {event_id_attr})"
        )
            ...
            for handler in self._handlers[contract_event_type]:
                ...
                try:
                    await handler.handle(
                        contract_event
                    )  # Assuming handlers have an async 'handle' method
                except Exception as e:
                    logger.error(
                        f"[IntegrationEventDispatcher] Error in integration handler '{handler_name}' "
                        f"for contract '{contract_event_type.__name__}': {e}",
                        exc_info=True,
                    )
        else:
            logger.debug(
                f"[IntegrationEventDispatcher] No handlers registered for public contract '{contract_event_type.__name__}'"
            )
